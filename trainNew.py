...
for epoch in range(num_epochs):
    ...
    for images, captions in dataloader:
        # Так как batch_size=1, получаем списки длиной 1
        image = images[0].unsqueeze(0).to(device)  # Добавляем измерение батча
        caption = captions[0].unsqueeze(0).to(device)
        optimizer.zero_grad()
        # Пропускаем изображение через кодировщик
        memory = encoder(image)  # [seq_len_enc, batch_size, embed_dim]
        # Подготавливаем входы и цели для декодера
        tgt_input = caption[:, :-1].transpose(0, 1)  # [seq_len_dec, batch_size]
        tgt_output = caption[:, 1:].transpose(0, 1)  # [seq_len_dec, batch_size]
